...
def run_case_par(seq,shell,nist_cutoff, prior_shape,likelihood_shape, cent_pot,n_walkers,n_steps,up_dir,nist_shift,grid_resolution,x_bnd,egy_dist,atom):
    ...
    if (egy_dist == 1):
       ...
       E_samples, Err, Erg=energy_distribution(ion, up_dir,emax, lambda_samples, x_bnd, x_res, cent_pot)
        ...
        E_samples='Option for energy distribution function was turned off, run again with egy_dist =0'
        Err='Option for energy distribution function was turned off, run again with egy_dist =0'
        Erg='Option for energy distribution function was turned off, run again with egy_dist =0'
    ...
    result=[T,nist_vals,emax,Erg,x_res,X_1D,lambda_samples,E_samples,Err,accept,autocorrel,rate_samples,rates]
    return result